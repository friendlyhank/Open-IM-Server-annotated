...
type FriendRequest struct {
	FromUserID    string    `gorm:"column:from_user_id;primary_key;size:64"` // 来源用户id
	ToUserID      string    `gorm:"column:to_user_id;primary_key;size:64"`   // 请求用户id
	HandleResult  int32     `gorm:"column:handle_result"`                    // 处理结果
	ReqMsg        string    `gorm:"column:req_msg;size:255"`                 // 请求备注信息
	CreateTime    time.Time `gorm:"column:create_time"`                      // 创建时间
	HandlerUserID string    `gorm:"column:handler_user_id;size:64"`          // 处理用户id
	HandleMsg     string    `gorm:"column:handle_msg;size:255"`              // 处理信息
	HandleTime    time.Time `gorm:"column:handle_time"`                      // 处理时间
	Ex            string    `gorm:"column:ex;size:1024"`                     // 额外信息
}
